...
function _ai_gemini_agent() {
  # 1. Check for API key. Fail early if it's not set. 
  if [[ -z "$GEMINI_API_KEY" ]]; then
    echo "Error: GEMINI_API_KEY environment variable is not set." >&2 
    return 1
  fi

  # 2. Get the prompt from the first argument. 
  local PROMPT
  PROMPT="$1"
  if [[ -z "$PROMPT" ]]; then
    echo "Usage: [cat context |] _ai_gemini_agent \"Your prompt\"" >&2 
    return 1
  fi

  # 3. Get the context from stdin (if it's being piped in). 
  local CONTEXT
  if [ ! -t 0 ]; then
    CONTEXT=$(cat)
  fi

  # 4. Build the combined text payload. 
  local PAYLOAD_TEXT
  if [[ -n "$CONTEXT" ]]; then
    # \n is a separator.
  ...
  # 5. Build the JSON payload using jq for safety. 
  local JSON_PAYLOAD
  JSON_PAYLOAD=$(jq -n \
    --arg text "$PAYLOAD_TEXT" \
    '{contents: [{parts: [{text: $text}]}]}')

  # 6. Call the Gemini API and parse the response. 
  curl -s \
...
function review-diff() {
  ...
  Provide a list of concise, actionable feedback." 
}

# 2. Review a specific commit
function review-commit() {
  git show $1 | _ai_gemini_agent "You are a code reviewer. Analyze this commit ($1) for bugs, style issues, or bad naming." 
}
...
function name-branch() {
  ...
  Suggest 3 short, descriptive git branch names using 'type/description' format (e.g., 'feature/user-login', 'bugfix/payment-crash')." 
}
...
function review-security() {
  ...
  List any potential issues and suggest a fix." 
}
...
function check-naming() {
  ...
  For each bad name, suggest a better one that describes its domain purpose." 
}
...
function refactor-readability() {
  ...
  Provide 'before' and 'after' snippets for your suggestions." 
}
...
function check-pr-description() {
  ...
  Rewrite it to be a perfect PR description." 
}
...
function write-failing-test() {
  ...
  Return only the complete, new contents of the test file." 
}
...
function write-unit-tests() {
  ...
  - Return only the complete contents of the test file." 
}
...
function write-integration-test() {
  ...
  - Focus on the data flow and contracts between the provided files." 
}
...
function fix-failing-test() {
  ...
  Write ONLY the complete, new contents of the modified implementation file." 
}
...
function harden-tests() {
  ...
  - Add tests for expected exceptions (e.g., 'pytest.raises')." 
}
...
function write-mock-patch() {
  ...
  Provide a brief example of its use in a test function." 
}
...
function implement-function() {
  ...
  Return the complete file content, with the function implemented." 
}
...
function write-github-action() {
  ...
  - Define the jobs, steps, and required 'uses' actions (e.g., 'actions/checkout@v3')." 
}
...
function write-regex() {
  ...
  Provide the regex, an explanation of its parts, and examples in Python." 
}
...
function implement-spec() {
  ...
  - Return the complete, single file." 
}
...
function refactor-to-function() {
  ...
  The new function name should be: '$1'." 
}
...
function refactor-conditional() {
  ...
  - De Morgan's Law
  Return just the refactored code block." 
}
...
function explain-code() {
  ...
  - Is there a 'gotcha' or hidden complexity?" 
}
...
function write-readme-section() {
  ...
  - Keep it concise.
  - Match the existing README's markdown style." 
}
...
function explain-error() {
  ...
  4. Suggest 3 possible ways to fix it." 
}
...
function debug-this() {
  ...
  4. What is the most likely file/function causing the issue?" 
}
...
function critique-design() {
  ...
  - What 3 questions should I ask the author?" 
}
...
function draw-diagram() {
  ...
  Return *only* the complete, valid diagram syntax." 
}
...
function define-api() {
  ...
  5. An example JSON 400 Error response." 
}
...
function write-sql() {
  ...
  - The dialect is '$2' (e.g., 'PostgreSQL')." 
}
...
function optimize-sql() {
  ...
  3. Rewrite the query if an index isn't enough." 
}
...
function explain-shell() {
  ...
  - What does it do?
  - Is there a simpler or more modern alternative (e.g., using 'rg' instead of 'grep')?" 
}
...
function find-code() {
  ...
  if [[ "$1" == "help" || "$1" == "--help" ]]; then
    echo "Usage: ls -R | _ai find-code \"search-term\""
    echo "Generates a 'grep' or 'rg' command to find code."
    return 0
  fi
...
function _ai_show_single_help() {
  ...
  case "$CMD" in
    ...
    "check-naming") echo "  _ai check-naming <file-path>" && echo "  Checks a file for bad naming conventions." ;; 
    "refactor-readability") echo "  _ai refactor-readability <file-path>" && echo "  Critiques a file's overall readability." ;; 
    "check-pr-description") echo "  _ai check-pr-description <pr-desc.txt>" && echo "  Reviews a PR description text file." ;; 
    "write-test-scenarios") echo "  cat code.py | _ai write-test-scenarios" && echo "  Generates human-readable test scenarios for a file." ;; 
    "write-failing-test") echo "  cat code.py | _ai write-failing-test \"feature\" \"pytest\"" && echo "  Writes a new failing TDD test." ;; 
    "write-unit-tests") echo "  cat code.py | _ai write-unit-tests \"pytest\"" && echo "  Writes a full unit test suite for a file." ;; 
    "write-integration-test") echo "  (cat f1.py; cat f2.py) | _ai write-integration-test \"pytest\"" && echo "  Writes an integration test for multiple files." ;; 
    "fix-failing-test") echo "  (cat code.py; cat test.py; pytest) | _ai fix-failing-test" && echo "  Fixes implementation code to pass a failing test." ;; 
    "harden-tests") echo "  _ai harden-tests <test_file.py>" && echo "  Adds edge cases to a test file." ;; 
    "write-mock-patch") echo "  cat module.py | _ai write-mock-patch \"func_name\" \"pytest\"" && echo "  Shows how to mock a function in a file." ;; 
    "write-e2e-script") echo "  cat story.txt | _ai write-e2e-script \"Cypress\"" && echo "  Writes an E2E script from a user story." ;; 
    "scaffold-module") echo "  _ai scaffold-module \"Python\" \"UserService class\"" && echo "  Scaffolds a new class or module." ;;
    "implement-function") echo "  cat stub.py | _ai implement-function" && echo "  Implements a function from a stub." ;; 
    "write-dockerfile") echo "  ls -R | _ai write-dockerfile \"python-flask\"" && echo "  Writes a Dockerfile based on file tree." ;; 
    "write-github-action") echo "  ls -R | _ai write-github-action \"run pytest\"" && echo "  Writes a GitHub Actions workflow YAML." ;; 
    ...
    "implement-spec") echo "  _ai implement-spec <api-spec.yml> \"FastAPI\"" && echo "  Generates server boilerplate from a spec." ;; 
    "refactor-dry") echo "  _ai refactor-dry <file.py>" && echo "  Refactors a file to remove duplicated code." ;; 
    ...
    "refactor-to-class") echo "  _ai refactor-to-class <script.py> \"NewClassName\"" && echo "  Converts a procedural script to a class." ;; 
    "refactor-conditional") echo "  cat code.txt | _ai refactor-conditional" && echo "  Simplifies a complex conditional snippet." ;;
    "refactor-to-async") echo "  _ai refactor-to-async <file.py> \"Python Flask\"" && echo "  Converts a synchronous file to async." ;; 
    "write-aboutme") echo "  _ai write-aboutme <file.py>" && echo "  Writes the 2-line ABOUTME header for a file." ;; 
    "write-docstrings") echo "  cat file.py | _ai write-docstrings \"google\"" && echo "  Adds docstrings to all functions in a file." ;; 
    "explain-code") echo "  cat code.txt | _ai explain-code" && echo "  Explains a piped-in block of code." ;;
    "write-readme-section") echo "  (cat README.md; cat f.py) | _ai write-readme-section \"Usage\"" && echo "  Generates a new README section." ;; 
    "fix-comment") echo "  _ai fix-comment <file.py>" && echo "  Finds and fixes an incorrect comment in a file." ;; 
    "explain-error") echo "  cat error.log | _ai explain-error" && echo "  Explains a piped-in stack trace." ;;
    "analyze-log") echo "  cat server.log | _ai analyze-log \"find 500 errors\"" && echo "  Analyzes a large log file for a prompt." ;; 
    "debug-this") echo "  (cat code.py; cat report.txt) | _ai debug-this" && echo "  Suggests debugging steps for a bug." ;;
    "find-bug-commit") echo "  (git log -p; cat err.log) | _ai find-bug-commit" && echo "  Finds the bug-introducing commit from a log." ;; 
    "write-debugger-command") echo "  cat code.py | _ai write-debugger-command \"pdb\" \"break on 42\"" && echo "  Writes the exact debugger commands." ;; 
    "critique-design") echo "  _ai critique-design <design.md>" && echo "  Critiques a design document for pragmatism." ;; 
    ...
    "write-sql") echo "  cat schema.sql | _ai write-sql \"prompt\" \"PostgreSQL\"" && echo "  Writes a SQL query from a schema and prompt." ;; 
    "optimize-sql") echo "  (cat s.sql; cat q.sql; cat p.txt) | _ai optimize-sql" && echo "  Optimizes a slow SQL query using its query plan." ;; 
    "write-migration") echo "  (cat before.py; cat after.py) | _ai write-migration \"alembic\"" && echo "  Writes a DB migration script." ;; 
    ...
    "find-code") echo "  ls -R | _ai find-code \"prompt\"" && echo "  Generates a 'grep' command to find code." ;; 
